...
//        soyReal.mul(new Real( String.valueOf(parametro * ( ALTO_PANTALLA - y ) / ALTO_PANTALLA)));
    ...
    public int recta( int y, float pendiente){         //Parámetro marca que tan marcada es la pendiente de la recta
        //Mi súper mega función patentada por la UNAM, corolario 1.
        return Posicionador.redondearEntero( pendiente * (ALTO_PANTALLA - y) / ALTO_PANTALLA );
    }

    //Desarrollamos el posicionador para casos potenciales
    public int potencial( int y, float parametro, int exponente){
        //Mi súper mega función patentada por la UNAM, corolario 2.
        return Posicionador.redondearEntero( ( parametro * Posicionador.potencia( (double) (ALTO_PANTALLA - y / ALTO_PANTALLA ) , exponente ) * ( ALTO_PANTALLA - y ) / ALTO_PANTALLA ) );
    }

    //Desarrollamos un posicionador en función de mla reducción del camino
    public int incrementoAncho( int y, int anchoInicial, int porcentajeAnchoFinal, int alturaFondo){
        //Mi súper mega función patentada por el IPN. Sin corolarios.
        return Posicionador.redondearEntero( (float)anchoInicial*((float)1-(((float)100-(float)porcentajeAnchoFinal)/(float)100)*((float)(ALTO_PANTALLA-y)/(float)(ALTO_PANTALLA-alturaFondo))) );
    }
